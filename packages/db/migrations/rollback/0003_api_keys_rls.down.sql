...
DROP POLICY IF EXISTS "api_key_resolver_touch" ON "api_keys";
DROP POLICY IF EXISTS "api_key_resolver_read" ON "api_keys";
...
REVOKE ALL ON "api_keys" FROM api_key_resolver;
-- Cluster-wide role: DROP ROLE fails while another database still references it.
DROP ROLE IF EXISTS api_key_resolver;
